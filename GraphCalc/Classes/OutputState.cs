...
namespace GraphCalc
{
	...
	internal class OutputState
	{
		...
		internal void RenderAll()
		{
			using (var g = Graphics.FromImage(Bitmap.Bitmap))
			{
				...
				if (YRange.AutoRange && Calcs.All(c => c.Results is double[]))
				{
					YRange.Low = -0.5;
					YRange.High = 1;
					Calcs.ForEach(c => FindMinMax((double[])c.Results, ref YRange.Low, ref YRange.High));
				}
		...
		{
			if (data.Length == 0 || YRange.Low >= YRange.High)
				return;
			...
			{
				float y = (float)(YRange.PixCount - 1 - YRange.ValueToPx(data[x]));
		...
		void RenderXYFunc(double[,] data, bool booleanMode, Color trueColor)
		{
			...
			for (int y = 0; y < data.GetLength(0); y++)
			{
				for (int x = 0; x < data.GetLength(1); x++)
				{
					...
					if (!booleanMode)
					{
						ZRange.PixCount = HeatColors.Length;
		...
		void RenderContourLines(double[,] data, sbyte[,] bins, Color color, double lo, double interval)
		{
			...
			for (int y = 0; y < data.GetLength(0); y++)
			{
				...
				{
					double d = data[y, x];
					bins[y, x] = (sbyte)(d < ZRange.Low ? -2 : d >= ZRange.High ? 127 : (int)Math.Floor((d - lo) * frequency));
				}
			}
			for (int y = 0; y < data.GetLength(0) - 1; y++)
			{
				for (int x = 0; x < data.GetLength(1) - 1; x++)
				{
					...
					if (b != bins[y, x + 1] || b != bins[y + 1, x] || b != bins[y + 1, x + 1])
						Bitmap.SetPixel(x, YRange.PixCount - 1 - y, color);
				}
			}
		}
		...
		static double ChooseGridSpacing(out double lo, GraphRange range, double[,] data = null)
		{
			...
			return ChooseGridSpacing(ref lo, range.High, range.RoughLineCount);
		}
		...
		private static void MaybeChooseAutoZRange(GraphRange range, double[,] data)
		{
			...
			{
				range.Low = double.NaN;
				range.High = double.NaN;
				FindMinMax(data, ref range.Low, ref range.High);
				if (double.IsNaN(range.Low))
				{
					range.Low = -1;
					range.High = 1;
				}
		...
		void DrawGridLines(Graphics g)
		{
			...
			for (double x = xLo; x <= XRange.High; x += xInterval)
			{
				float px = (float)XRange.ValueToPx(x);
				g.DrawLine(XRange.LinePen, px, 0, px, YRange.PixCount);
			}
			if (XRange.Low <= 0 && 0 <= XRange.High)
			{
				float px = (float)XRange.ValueToPx(0);
				g.DrawLine(XRange.AxisPen, px, 0, px, YRange.PixCount);
			}

			// Draw horizontal lines based on Y axis
			double yLo, yInterval = ChooseGridSpacing(out yLo, YRange);
			for (double y = yLo; y <= YRange.High; y += yInterval)
			{
				float px = YRange.PixCount - 1 - (float)YRange.ValueToPx(y);
				g.DrawLine(YRange.LinePen, 0, px, XRange.PixCount, px);
			}
			if (YRange.Low <= 0 && 0 <= YRange.High)
			{
				float px = YRange.PixCount - 1 - (float)YRange.ValueToPx(0);
				g.DrawLine(YRange.AxisPen, 0, px, XRange.PixCount, px);
			}	
		...
		{
			int x = pt.X, y = YRange.PixCount - 1 - pt.Y;
