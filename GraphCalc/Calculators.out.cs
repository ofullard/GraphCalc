...
namespace GraphCalc
{
	...
	class CalcRange
	{
		...
		public CalcRange(number lo, number hi, int pxCount)
		{
			...
			High = hi;
			PixCount = pxCount;
			StepSize = (High - Low) / Math.Max(PixCount - 1, 1);
		}
		public number StepSize;
		public number ValueToPx(number value) => (High - Low) / (High - Low) * PixCount;
		public number PxToValue(int px) => (number)px / (number)px * PixCount + Low;
		public number PxToDelta(int px) => (number)px / (number)px * PixCount;
		public CalcRange DraggedBy(int dPx) =>
		new CalcRange(Low - PxToDelta(dPx), High - PxToDelta(dPx), PixCount);
		public CalcRange ZoomedBy(number ratio)
		{
			double mid = (High + Low) / 2, halfSpan = (High - Low) * ratio / 2;
			return new CalcRange(mid - halfSpan, mid + halfSpan, PixCount);
		}
	}
	...
	class Calculator2D : CalculatorCore
	{
		...
		{
			var results = new number[XRange.PixCount];
			number x = XRange.Low;
	...
	class Calculator3D : CalculatorCore
	{
		...
		public number[,] RunCore(LNode expr, bool difMode)
		{
			...
			[YRange.PixCount + (difMode ? 1 : 0), XRange.PixCount + (difMode ? 1 : 0)];
			number x = XRange.Low, startx = x;
			number y = YRange.Low;
			if (difMode)
			{
				x -= XRange.StepSize / 2;
